...
if __name__ == "__main__":
    ...
    multiprocessing.set_start_method('spawn') # or 'spawn'? ## changed to spawn for WSL
    ...
    parser.add_argument('--if_adjacent', default=False, action='store_true',
                        help='save/load adjacent-frame outputs when supported')
    parser.add_argument('--if_temporal', default=False, action='store_true',
                        help='save/load temporal outputs when supported')
    ...
    ### Added SAM Dataset support
    elif args.dataset in ['sam']:
        from dataset_sam import Dataset_sam
        sf_dataset = Dataset_sam(args)
    ## Added valueError for debugging
    else: 
    ...
    for k, batch in enumerate(data_loader):
        ...
        if args.if_save:
            if args.dataset == 'sam':
                path_flow = data['data_path'].replace('.npz', '_flow.npz')
            else:
            ...
            ## checking if flow is saving
            print(f"  flows nonzero rows: {np.count_nonzero(flows.sum(axis=1))}/{len(flows)}")
            print(f"  flows mean: {flows.mean(axis=0)}")
